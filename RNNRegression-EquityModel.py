...
def choose_stocks(context,bar_dict):
    ...
    enddatel = []
    enddatel1 = []
    for i in list(range(1,timestep+1)):
    ...
    #Generate New Data
    #print(len(MyFactors))
    X_input_raw = AllData.ix[:,0:(len(MyFactors)-1)]
    print(X_input_raw.shape)
    
    Y_input_raw = AllData.ix[:,len(MyFactors)]
    ...
    for stock in UniStock:
        ...
        if tempAllRes.shape[0] == timestep:
            AllFacRes = np.append(AllFacRes,tempAllRes)
            #print(AllFacRes.shape)
            AllRet = np.append(AllRet,np.array(Y_input_raw.loc[stock]))
            count = count + 1
            #print(count)
    #Already normalized, so no need to normalize agian here
    print(AllFacRes.shape)
    print(count, timestep,len(MyFactors))
    AllFacRes = np.reshape(AllFacRes, (count, timestep,len(MyFactors)))
    AllRet = np.reshape(AllRet, (count, timestep, 1))
    X = AllFacRes
    Y = AllRet
    ...
    print(len(MyFactors),X.shape,Y.shape)
    model.fit(X, Y, nb_epoch=200, batch_size=30, verbose=2)
    
    #Prepare data fro prediction
    AllStock = index_components('000300.XSHG') #this is the stock pool for selection
    stock = AllStock
    enddatel = []
    for i in list(range(1,1+timestep)):
    ...
    X_input_raw = AllData
    AllFacRes = np.array([])
    AllStockCodes = np.array([])
    count = 0
    for stock in UniStock:
        tempAllRes = X_input_raw.loc[stock].values
        if tempAllRes.shape[0] == timestep:
            AllStockCodes = np.append(AllStockCodes,stock)
            AllFacRes = np.append(AllFacRes,tempAllRes)
            count = count + 1
    #Already normalized, so no need to normalize agian here
    NewX = np.reshape(AllFacRes, (count, timestep, len(MyFactors)))
    
    Y_Pred = model.predict(NewX)
    Y_Pred = Y_Pred[:,0,:] #Get only the latest predicti
    print(Y_Pred.shape)
    Pred_Result = pd.DataFrame(Y_Pred,index = AllStockCodes)
    Pred_Result = Pred_Result.T.iloc[0]
    Pred_Result.sort(ascending = False)
    context.stocks = Pred_Result.index.values[0:15]
